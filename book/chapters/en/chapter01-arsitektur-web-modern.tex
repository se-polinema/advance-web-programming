...
\begin{enumerate}
  ...
  \item get to know Laravel's project folder structure (\file{routes/}, \file{app/Http/Controllers/}, \file{database/migrations/}) as an embodiment of the MVC pattern, then initialize a Git repository with the first \cmd{increment 1} commit.
\end{enumerate}
...
\begin{description}[leftmargin=0pt,style=nextline]
  \item[Monolith] An application whose layers, interface, business logic, and data access, all run in a single codebase deployed as a single unit. Often mistaken for ``messy code''; a well-structured monolith (e.g. following MVC) stays clean, its real opposite isn't ``modular'', it's ``distributed''.
  \item[Microservices] An architecture that splits an application into independent services, each deployed on its own, communicating over a network. Rule of thumb: only worth it once an organization has enough teams that each can own and operate its own service; a small team splitting its application just adds operational overhead.
\end{description}
\end{istilahpenting}
...
\begin{istilahpenting}
\begin{description}[leftmargin=0pt,style=nextline]
  \item[Serverless] A model of code execution as small functions that only run when triggered by an event, with no server process staying up continuously; a cloud provider manages the infrastructure. The name is misleading: the server still exists, it just isn't your responsibility. Because the process doesn't stay alive continuously, there's a \textit{cold start} delay when a function hasn't been called recently, and billing is per execution, not per hour a server stays on.
\end{description}
\end{istilahpenting}
...
\begin{istilahpenting}
\begin{description}[leftmargin=0pt,style=nextline]
  \item[MVC] The \textit{Model-View-Controller} pattern: the Model owns data, the View owns presentation, and the Controller owns the request flow between them. Practical rule for where code goes: touches data, goes in the Model; handles request flow, goes in the Controller; renders output, goes in the View. The framework doesn't enforce this (a query inside a view still runs), it's a discipline the code's author has to maintain.
\end{description}
\end{istilahpenting}
...
Laravel gives a consistent \term{MVC} structure from the first line of code;
...
building monolithic web applications. Django (Python) and Ruby on Rails
share a similar ``convention over configuration'' philosophy with Laravel.
Laravel's standout strength is its package ecosystem
...
\begin{istilahpenting}
\begin{description}[leftmargin=0pt,style=nextline]
  \item[Artisan] Laravel's built-in command-line interface for everyday development tasks: migrations, seeding, and boilerplate file generation. The full command list can be viewed anytime by running \artisan{php artisan list}; Artisan is strictly a development tool, it never runs while the application serves real users.
  \item[Migration] A PHP file that defines a database schema change in code, so the schema can be rebuilt consistently on any machine. Treat it like version control for the schema: a new change means a new migration file, never editing an old migration that has already run on another machine.
\end{description}
\end{istilahpenting}
...
\textbf{What to expect}: \cmd{composer create-project} prints a list of
packages being downloaded one by one, ending with a line like
\texttt{Application ready in simple-pos. You can now start using
Composer!}; \artisan{key:generate} prints
\texttt{INFO Application key set successfully.} Those last three commands
prepare the \file{.env} configuration file
...
\begin{tipbox}
\textbf{If \cmd{composer create-project} fails} with a message
mentioning the PHP version (e.g. \texttt{requires php \textasciicircum{}8.2}),
run
\cmd{php -v} to confirm the installed PHP version is 8.2 or higher;
Laravel 13 cannot be installed on an older PHP version.
\end{tipbox}
...
\textbf{What to expect}: \cmd{npm install} prints the number of packages
installed, ending with a line like \texttt{added N packages in Ns}, with
no \texttt{npm error} lines in between. This step isn't required to run Simple POS today; Laravel's default
...
\textbf{What to expect}: the list of migrations prints one by one, each
ending with \texttt{DONE}, with no error lines in between, followed by a
message that the seeder finished running. \artisan{migrate:fresh} runs every migration file from scratch (dropping
...
\textbf{What to expect}: a line reading \texttt{INFO Server running on
[http://127.0.0.1:8000]}, then the terminal stays open waiting for
requests (press \texttt{Ctrl+C} to stop it). Open that address in a
browser; Laravel's welcome page, not an error message, is the sign every
step in this section succeeded. That command serves the application at \route{http://127.0.0.1:8000} using
...
chapter, just in the form of real running code instead of prose. The
following steps create your own \cmd{increment 1} commit, so this
convention feels real, not just something you read about.

\begin{enumerate}
  \item \cmd{composer create-project} doesn't automatically set up a Git
    repository (unlike the \cmd{laravel new} installer, which does this
    automatically), so this step is done manually on purpose from inside
    the \file{simple-pos/} folder:
    \begin{lstlisting}[language=bash]
git init
    \end{lstlisting}
    \textbf{What to expect}: a message reading \texttt{Initialized empty
    Git repository in .../simple-pos/.git/}.
  \item Add every project file to the staging area, then make the first
    commit with a message following the \cmd{increment N} convention:
    \begin{lstlisting}[language=bash]
git add .
git commit -m "increment 1: empty Laravel project"
    \end{lstlisting}
    \textbf{What to expect}: a summary of how many files were committed,
    ending with a line like \texttt{N files changed, M insertions(+)}.
    Notice that \file{vendor/}, \file{node\_modules/}, and \file{.env}
    aren't mentioned at all; Laravel's default \file{.gitignore} already
    excludes all three, exactly as the warning in the previous section
    said.
  \item Verify the commit actually got saved:
    \begin{lstlisting}[language=bash]
git log --oneline
    \end{lstlisting}
    \textbf{What to expect}: one line with a short hash followed by the
    message \texttt{increment 1: empty Laravel project}, exactly as just
    written.
  \item \textbf{If \cmd{git commit} fails} with a message asking for
    \texttt{user.name}/\texttt{user.email}, this is a machine that has
    never used Git before; set it once with
    \cmd{git config --global user.name "Your Name"} and
    \cmd{git config --global user.email "email@you.com"}, then repeat
    step 2.
\end{enumerate}
...
through like a real commit history. The end of every Exercise section in
...
\begin{itemize}
  ...
  \item Laravel's folder structure (\file{routes/}, \file{app/Http/Controllers/},
    \file{database/migrations/}) embodies the MVC pattern; the first
    \cmd{increment N} commit marks the Git repository as initialized,
    with the same convention mapping the application's growth chapter by
    chapter and the full code for each chapter available as its own
    GitHub branch.
\end{itemize}
...
\begin{enumerate}
  \item Run every setup command from Section 2 (\cmd{composer
    create-project} through \artisan{serve}) on your own machine, then
    confirm \cmd{php artisan serve} shows Laravel's welcome page at
    \route{http://127.0.0.1:8000}; follow up with the three Git commands
    from Exercise 1.2 and confirm \cmd{git log --oneline} shows your own
    \texttt{increment 1} commit.
