...
\begin{enumerate}
  ...
  \item mengenali struktur folder proyek Laravel (\file{routes/}, \file{app/Http/Controllers/}, \file{database/migrations/}) sebagai perwujudan pola MVC, lalu menginisialisasi repositori Git dengan commit \cmd{increment 1} pertama.
\end{enumerate}
...
\begin{description}[leftmargin=0pt,style=nextline]
  \item[Monolith] Aplikasi yang seluruh lapisannya, antarmuka, logika bisnis, dan akses data, berjalan dalam satu basis kode dan satu proses deploy. Sering disalahpahami sebagai ``kode berantakan''; monolith yang terstruktur (mis. dengan MVC) tetap rapi, lawan katanya bukan ``modular'', melainkan ``terdistribusi''.
  \item[Microservices] Arsitektur yang memecah aplikasi menjadi layanan-layanan independen, masing-masing berjalan dan di-deploy sendiri, saling berkomunikasi lewat jaringan. Aturan praktis: baru sepadan ketika organisasi punya banyak tim yang masing-masing bisa memiliki dan mengoperasikan layanannya sendiri; satu tim kecil yang memecah aplikasinya hanya menambah ongkos operasional.
\end{description}
\end{istilahpenting}
...
\begin{istilahpenting}
\begin{description}[leftmargin=0pt,style=nextline]
  \item[Serverless] Model eksekusi kode sebagai fungsi-fungsi kecil yang hanya berjalan saat dipicu peristiwa, tanpa proses server yang menyala terus-menerus; penyedia cloud yang mengurus infrastrukturnya. Namanya menyesatkan: server tetap ada, hanya bukan tanggung jawabmu. Karena proses tidak hidup terus-menerus, ada jeda \textit{cold start} saat fungsi lama tidak dipanggil, dan biaya dihitung per eksekusi, bukan per jam server menyala.
\end{description}
\end{istilahpenting}
...
\begin{istilahpenting}
\begin{description}[leftmargin=0pt,style=nextline]
  \item[MVC] Pola \textit{Model-View-Controller}: Model mengurus data, View mengurus tampilan, Controller mengurus alur permintaan di antara keduanya. Aturan praktis menaruh kode: menyentuh data ke Model, mengatur alur permintaan ke Controller, menampilkan ke View. Framework tidak memaksa ini, query di dalam view tetap bisa jalan, pola ini adalah disiplin yang dijaga penulis kodenya sendiri.
\end{description}
\end{istilahpenting}
...
memberi struktur \term{MVC} yang konsisten sejak baris kode pertama; dibandingkan
...
Satu detail yang membedakan Laravel dari PHP polos: pada PHP polos, alamat
URL biasanya sama persis dengan path berkas di server (mis.
\route{/produk.php} memuat \file{produk.php}). Laravel meniadakan pemetaan
langsung ini; setiap permintaan, apa pun alamatnya, selalu masuk lewat satu
berkas yang sama, \file{public/index.php}, yang baru kemudian menyerahkan
permintaan itu ke \file{routes/web.php} untuk dicocokkan dengan Controller
yang sesuai. Tidak ada berkas lain di dalam proyek yang bisa diakses
langsung lewat URL. Mulai Bab~2, kamu juga akan melihat pemanggilan seperti
\cmd{Route::get(...)}; sekilas mirip pemanggilan method static PHP biasa,
tetapi proses di baliknya berbeda (disebut \textit{facade}), dijelaskan
lebih lanjut saat pertama kali dipakai.

\begin{notebox}
Laravel bukan satu-satunya kerangka kerja PHP, dan PHP bukan satu-satunya
bahasa untuk membangun aplikasi web monolith. Django (Python) dan Ruby on
Rails punya filosofi ``convention over configuration'' yang mirip Laravel.
Posisi Laravel yang menonjol ada pada ekosistem
...
\begin{tipbox}
Sebelum lanjut, pastikan empat alat berikut sudah terpasang dengan
menjalankan masing-masing perintah ini di terminal: \cmd{php -v} (versi
8.2 ke atas), \cmd{composer -V}, \cmd{node -v}, dan \cmd{git --version}.
Kalau salah satu belum terpasang atau versinya terlalu lama, instal dulu
sebelum melanjutkan ke langkah berikutnya.
\end{tipbox}

\begin{istilahpenting}
\begin{description}[leftmargin=0pt,style=nextline]
  \item[Artisan] Antarmuka baris perintah bawaan Laravel untuk menjalankan tugas pengembangan sehari-hari: migrasi, seeding, dan pembuatan berkas boilerplate. Daftar lengkap perintahnya bisa dilihat kapan saja dengan menjalankan \artisan{php artisan list}; Artisan hanya alat bantu pengembangan, tidak ikut berjalan saat aplikasi melayani pengguna. Daftar perintah Artisan dan Composer yang paling sering dipakai di buku ini terangkum di Lampiran~B.
  \item[Migrasi] Berkas PHP yang mendefinisikan perubahan skema basis data secara terprogram, sehingga skema dapat dibangun ulang secara konsisten di komputer mana pun. Perlakukan seperti version control untuk skema: perubahan baru berarti berkas migrasi baru, bukan mengedit migrasi lama yang sudah pernah dijalankan di komputer lain.
\end{description}
\end{istilahpenting}
...
\textbf{Yang diharapkan}: \cmd{composer create-project} mencetak daftar
paket yang diunduh satu per satu, diakhiri baris seperti
\texttt{Application ready in simple-pos. You can now start using
Composer!}; \artisan{key:generate} mencetak
\texttt{INFO Application key set successfully.} Tiga perintah terakhir
menyiapkan berkas konfigurasi \file{.env} (tempat kredensial dan
pengaturan lingkungan disimpan, terpisah dari kode sumber supaya tidak
pernah ikut ter-commit ke Git) dan menghasilkan kunci enkripsi aplikasi
yang dipakai Laravel untuk mengamankan sesi dan cookie.

\cmd{composer create-project} bukan sekadar mengunduh Laravel, melainkan
menjalankan Composer, pengelola dependensi PHP: ia membaca berkas
\file{composer.json}, mengunduh setiap paket yang terdaftar ke
\file{vendor/}, lalu menghasilkan \file{vendor/autoload.php}. Berkas
itulah yang membuat setiap class di proyek ini, termasuk milikmu sendiri,
langsung bisa dipakai tanpa \cmd{require} atau \cmd{include} manual
seperti pada PHP polos. Nilai di \file{.env} juga tidak langsung dipakai
di kode; Laravel membacanya lewat helper \cmd{env()}, yang biasanya
dibungkus lagi oleh berkas \file{config/*.php} dan diakses lewat helper
\cmd{config()}. Lapisan ini tidak ada padanannya di PHP polos yang
biasanya hanya memakai konstanta atau berkas \texttt{.ini} tunggal, tetapi
memungkinkan konfigurasi di-cache untuk performa saat aplikasi berjalan
di server produksi (dibahas lebih lanjut pada Bab~11).

\begin{tipbox}
\textbf{Kalau \cmd{composer create-project} gagal} dengan pesan yang
menyinggung versi PHP (mis. \texttt{requires php \textasciicircum{}8.2}),
jalankan
\cmd{php -v} untuk memastikan versi PHP yang terpasang 8.2 ke atas;
Laravel 13 tidak bisa dipasang di versi PHP yang lebih lama.
\end{tipbox}
...
\textbf{Yang diharapkan}: \cmd{npm install} mencetak jumlah paket yang
terpasang, diakhiri baris seperti \texttt{added N packages in Ns}, tanpa
baris \texttt{npm error} di antaranya. Langkah ini belum wajib untuk
menjalankan Simple POS hari ini; halaman bawaan Laravel masih tampil
dengan gaya cadangan meski \file{node\_modules/} belum ada.
Menjalankannya sekarang menghindari jeda instalasi mendadak begitu Bab~3
mulai memakai Tailwind dan Alpine.js lewat Vite. Langkah berikutnya
mengarahkan koneksi basis data ke sebuah berkas SQLite:
...
\textbf{Yang diharapkan}: daftar migrasi tercetak satu per satu, masing-masing
diakhiri kata \texttt{DONE}, tanpa satu pun baris error di antaranya,
diikuti pesan bahwa seeder selesai berjalan. \artisan{migrate:fresh}
menjalankan seluruh berkas migrasi dari awal (menghapus tabel lama jika
ada, lalu membangunnya kembali dari definisi skema terbaru), sedangkan
opsi \cmd{--seed} langsung mengisi tabel yang baru dibuat dengan data
contoh lewat berkas seeder. Untuk Simple POS, seeder ini mengisi ratusan
produk dan ribuan transaksi sekaligus, sesuatu yang akan dibahas lebih
dalam pada Bab~4 ketika alasan mengisi data dalam skala nyata, bukan
sekadar beberapa baris, mulai terlihat manfaatnya.
...
\textbf{Yang diharapkan}: baris \texttt{INFO Server running on
[http://127.0.0.1:8000]}, lalu terminal tetap terbuka menunggu request
(tekan \texttt{Ctrl+C} untuk menghentikannya). Buka alamat itu di
browser; halaman selamat datang Laravel, bukan pesan error, adalah
tanda seluruh langkah di bagian ini berhasil. Perintah ini menjalankan
aplikasi pada \route{http://127.0.0.1:8000} memakai server PHP bawaan,
cukup untuk seluruh pengembangan lokal di buku ini. Server produksi
sungguhan (dibahas pada Bab~11) memakai konfigurasi berbeda, tetapi
struktur aplikasi yang dipelajari di sini tetap sama persis.
...
hanya dalam bentuk kode sungguhan alih-alih prosa. Langkah-langkah berikut
membuat commit \cmd{increment 1} milikmu sendiri, supaya konvensi ini
terasa nyata, bukan cuma dibaca.

\begin{enumerate}
  \item \cmd{composer create-project} tidak otomatis menyiapkan repositori
    Git (berbeda dari installer \cmd{laravel new} yang melakukannya
    otomatis), jadi langkah ini sengaja dilakukan manual dari dalam folder
    \file{simple-pos/}:
    \begin{lstlisting}[language=bash]
git init
    \end{lstlisting}
    \textbf{Yang diharapkan}: pesan \texttt{Initialized empty Git
    repository in .../simple-pos/.git/}.
  \item Tambahkan seluruh berkas proyek ke area staging, lalu buat commit
    pertama dengan pesan yang mengikuti konvensi \cmd{increment N}:
    \begin{lstlisting}[language=bash]
git add .
git commit -m "increment 1: proyek Laravel kosong"
    \end{lstlisting}
    \textbf{Yang diharapkan}: ringkasan jumlah berkas yang di-commit,
    diakhiri baris seperti \texttt{N files changed, M insertions(+)}.
    Perhatikan bahwa \file{vendor/}, \file{node\_modules/}, dan
    \file{.env} tidak ikut disebutkan; \file{.gitignore} bawaan Laravel
    sudah mengecualikan ketiganya, persis seperti peringatan pada bagian
    sebelumnya.
  \item Verifikasi commit itu benar-benar tersimpan:
    \begin{lstlisting}[language=bash]
git log --oneline
    \end{lstlisting}
    \textbf{Yang diharapkan}: satu baris berisi hash pendek diikuti pesan
    \texttt{increment 1: proyek Laravel kosong} persis seperti yang baru
    saja ditulis.
  \item \textbf{Kalau \cmd{git commit} gagal} dengan pesan yang meminta
    \texttt{user.name}/\texttt{user.email}, ini komputer yang belum
    pernah dipakai Git sebelumnya; atur sekali saja dengan
    \cmd{git config --global user.name "Nama Kamu"} dan
    \cmd{git config --global user.email "email@kamu.com"}, lalu ulangi
    langkah 2.
\end{enumerate}
...
\begin{itemize}
  ...
  \item Struktur folder Laravel (\file{routes/}, \file{app/Http/Controllers/},
    \file{database/migrations/}) mewujudkan pola MVC; commit
    \cmd{increment N} pertama menandai repositori Git terinisialisasi,
    dengan konvensi yang sama memetakan pertumbuhan aplikasi bab demi bab
    dan kode lengkap tiap bab tersedia sebagai branch tersendiri di
    GitHub.
\end{itemize}
...
\begin{enumerate}
  \item Jalankan seluruh perintah setup pada Bagian 2 (\cmd{composer
    create-project} sampai \artisan{serve}) di komputermu sendiri, lalu
    verifikasi bahwa \cmd{php artisan serve} menampilkan halaman selamat
    datang Laravel di \route{http://127.0.0.1:8000}; lanjutkan dengan
    ketiga perintah Git pada Praktik 1.2 dan pastikan \cmd{git log
    --oneline} menampilkan commit \texttt{increment 1} milikmu.
